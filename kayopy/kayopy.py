    ...
    if vstup.isnumeric():
        ...
        try:
            if args.AutoDownload is None:
                vstup = input(f"Do you want to download content of folder? (Y/n) > ")
                if vstup in ["", "Y", "y"]:
...
class ParseSite:
    ...
    def get(self, __name: str) -> any:
        if __name == "dow_link":
            if self.type == "casual":
                items = self.parser.find_all("a", class_=["shortc-button", "small"])
                return items
            elif self.type == "list":
...
def main():
    ...
    while True:
        ...
        if vstup == "grec":
            ...
            vstup = input("Select anime > ")
            if vstup.isnumeric():
                links = ParseSite(recomendations[int(vstup) - 1]["href"]).get(
                    "dow_link"
                )
                if len(links) == 1:
                    print(links[0].text)
